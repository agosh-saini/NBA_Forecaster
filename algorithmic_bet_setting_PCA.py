...
def game_diff_data(dataset_team_stats, dataset_games, abrs):
    coloumns = {'PER': [], 'TS%': [], '3PAr': [], 'FTr': [], 'ORB%': [], 'AST%': [], 'TOV%': [], 'PT': [], 'W/L': []}
    df = pd.DataFrame(coloumns)
    ...
    for i in range(len(dataset_games)-1):
        ...
        data = {'PER': per, 'TS%': ts, '3PAr': threePar, 'FTr': ftr, 'ORB%': orb, 'AST%': ast, 'TOV%': tov, 'PT': pt, 'W/L': w_l}
        
        if np.size(per) == 0 or np.size(ts) == 0 or np.size(threePar) == 0 or np.size(pt) == 0 or np.size(ftr) == 0 or np.size(orb) == 0 or np.size(ast) == 0 or np.size(tov) == 0 or np.size(w_l) == 0:
            continue
...
three_year_game_data.to_csv(r'Stats\generated_files\three_year_game_data.csv')

three_year_tm_stats = pd.concat([tm_stats_16_17, tm_stats_17_18, tm_stats_18_19]).dropna()

three_year_tm_stats.to_csv(r'Stats\generated_files\three_year_tm_data.csv')
